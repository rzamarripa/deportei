...
function PruebasCtrl($scope, $meteor, $reactive, $state, toastr, $stateParams) {
	...
	this.subscribe('deportes', () => {
		...
		return [{
			evento_id: this.getReactively('buscar.buscarEvento_id') ? this.getReactively('buscar.buscarEvento_id') : "",
			estatus: true
		}]
	});
	...
	this.obtenerDeportes = function () {
		rc.deportes = [];
		Meteor.call("getDeportesEvento", this.prueba.evento_id, function (error, result) {
			if (error) {
				console.log(error);
	...
	this.obtenerCategorias = function () {
		rc.categorias = [];
		Meteor.call("getCategoriasEvento", this.prueba.evento_id, this.prueba.deporte_id , function (error, result) {
			if (error) {
				console.log(error);
				toastr.error('Error al obtener los datos.: ', error.details);
				//loading(false);
